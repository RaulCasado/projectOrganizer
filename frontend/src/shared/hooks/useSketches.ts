...
export const useSketches = (options: UseSketchesOptions = {}) => {
  ...
  const [storageInfo, setStorageInfo] = useState<SketchStorageInfo | null>(null);
  ...
  const createSketch = useCallback(async (
    ...
    try {
      ...
      const newSketch = sketchStorageService.saveSketch(canvas, completeMetadata);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error creando sketch';
  ...
  const deleteSketch = useCallback((id: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error eliminando sketch';
      setError(errorMessage);
      return false;
    }
  }, [loadSketches]);
  ...
  const exportSketch = useCallback((id: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Error exportando sketch';
      setError(errorMessage);
    }
  }, []);
