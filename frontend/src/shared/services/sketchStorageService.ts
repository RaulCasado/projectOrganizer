...
class SketchStorageService {
  ...
  saveSketch(
    ...
    try {
      ...
      LocalStorageService.set(
        `${this.config.storagePrefix}${sketch.id}`,
        sketch
      );
  ...
  getSketch(id: string): QuickSketch | null {
    try {
      const sketch = LocalStorageService.get<QuickSketch | null>(
        `${this.config.storagePrefix}${id}`,
        null
      );
      if (sketch) {
  ...
  getAllSketches(): QuickSketch[] {
    ...
    const sketchKeys = LocalStorageService.getKeysWithPrefix(
  ...
  getStorageInfo(): SketchStorageInfo {
    const allSketches = this.getAllSketches();
    const usedStorage = allSketches.reduce(
      (total, sketch) => total + sketch.metadata.estimatedSize,
      0
    );
    ...
    allSketches.forEach(sketch => {
      if (sketch.projectId) {
        sketchesByProject[sketch.projectId] =
          (sketchesByProject[sketch.projectId] || 0) + 1;
      }
    });
  ...
  autoCleanup(projectId?: string): number {
    ...
    const sortedSketches = DateUtils.sortByDate(
      sketches,
      'lastAccessed',
      'asc'
    );
