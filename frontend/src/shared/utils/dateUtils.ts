...
export class DateUtils {
    ...
    static formatShort(date: string | Date): string {        // "25/08/2024"
        const parsed = typeof date === 'string' ? parseISO(date) : date;
        return isValid(parsed) ? format(parsed, 'P', { locale: this.locale }) : '---';
    }

    static formatMedium(date: string | Date): string {       // "25 ago 2024" 
        const parsed = typeof date === 'string' ? parseISO(date) : date;
        return isValid(parsed) ? format(parsed, 'PP', { locale: this.locale }) : '---';
    }


    static formatLong(date: string | Date): string {         // "25 de agosto de 2024"
        const parsed = typeof date === 'string' ? parseISO(date) : date;
        return isValid(parsed) ? format(parsed, 'PPP', { locale: this.locale }) : '---';
    }

    static formatFull(date: string | Date): string {         // "domingo, 25 de agosto de 2024"
        const parsed = typeof date === 'string' ? parseISO(date) : date;
        return isValid(parsed) ? format(parsed, 'PPPP', { locale: this.locale }) : '---';
    }

    static formatTime(date: string | Date): string {         // "14:30"
        const parsed = typeof date === 'string' ? parseISO(date) : date;
        return isValid(parsed) ? format(parsed, 'p', { locale: this.locale }) : '---';
    }

    static formatDateTime(date: string | Date): string {     // "25 ago 2024, 14:30"
        const parsed = typeof date === 'string' ? parseISO(date) : date;
        return isValid(parsed) ? format(parsed, 'Pp', { locale: this.locale }) : '---';
    }
    ...
    static getWeekRange(date?: Date) {
        ...
        return {
            ...
            startFormatted: format(startOfWeek(target, { weekStartsOn: 1 }), 'yyyy-MM-dd'),
            endFormatted: format(endOfWeek(target, { weekStartsOn: 1 }), 'yyyy-MM-dd')
        };
    }
